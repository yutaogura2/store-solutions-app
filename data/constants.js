...
(function (root, factory) {
  ...
  return {
    ...
    /* --- アプリ版番号(表示の唯一の出所) ---
       画面ヘッダー・報告はここを参照する。package.json の version と一致させること(tests/version.test.js が検査)。
       保存JSONの _version はデータスキーマ版で別物 — 連動させない */
    APP_VERSION: "1.6.0",
    ...
    /* --- リース試算(概算・審査/料率により変動する旨を常時明記) ---
       月額 = 投資額(中央値)×月額料率。料率の既定値は一般的な目安 — 同社の提携リース条件に合わせて商談時に調整する */
    DEFAULT_LEASE_YEARS: 6,
    DEFAULT_LEASE_RATE_PERCENT: 1.7,  // 月額料率(%)。6年リースの一般的目安 — 要調整
    ...
    /* --- 標準工期の目安(現地調査で確定・同社の実務値に合わせて更新する) --- */
    WORK_DURATION_NOTES: {
      aircon: "1台あたり半日〜1日(夜間・休業日の施工も対応可)",
      lighting: "半日〜1日(営業時間外の施工も対応可)",
      kitchen: "半日程度(搬入経路により変動)",
      renovation: "内容により1〜7日程度(閑散期・夜間の分散施工も相談可)"
    }
  };
});
